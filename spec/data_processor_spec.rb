...
RSpec.describe DataProcessor do
  describe ".combine" do
    it "combines two CSV files into one YML file" do
      ...
      expect(File.read(filepath)).to eq(File.read("#{File.dirname(__FILE__)}/fixtures/combined.yml"))
    end
  end
  ...
  describe ".combine_raw" do
    it "combines two CSV parsed inputs into one array of Person" do
      people_data = CSV.parse(<<~CSV, headers: true)
        Email,Eligible for in-hours primary?,Can do in-hours secondary?,Should be scheduled for on-call?
        a@a.com,Yes,Yes,Yes
      CSV
